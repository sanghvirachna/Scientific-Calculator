...
import java.math.*;
import com.calculator.ThemeLoader;
...
import static com.calculator.ColorUtil.hex2Color;
...
public class CalculatorUI {
    ...
    private void initCalculatorTypeSelector() {
        comboCalculatorType = createComboBox(new String[]{"Standard", "Scientific","Base-N"}, 20, 30, "Calculator type");
    ...
    private void initButtons(int[] columns, int[] rows) {
        ...
        btnHEX = createButton("H", columns[2], rows[1]);
        btnHEX.addActionListener(event -> {
            if (!Pattern.matches(DOUBLE_OR_NUMBER_REGEX, inputScreen.getText()) || !go)
                return;
            int i = Integer.parseInt(inputScreen.getText());
        ...
        btnOCT = createButton("O", columns[3], rows[1]);
        btnOCT.addActionListener(event -> {
        	if (!Pattern.matches(DOUBLE_OR_NUMBER_REGEX, inputScreen.getText()) || !go)
                return;
            int i = Integer.parseInt(inputScreen.getText());
        ...
        btnBIN = createButton("B", columns[3], rows[2]);
        btnBIN.addActionListener(event -> {
        	if (!Pattern.matches(DOUBLE_OR_NUMBER_REGEX, inputScreen.getText()) || !go)
                return;
            int i = Integer.parseInt(inputScreen.getText());
        ...
        btnDEC = createButton("D", columns[3], rows[3]);
        btnDEC.addActionListener(event -> {
        	if (!Pattern.matches(DOUBLE_OR_NUMBER_REGEX, inputScreen.getText()) || !go)
                return;
        	String i = inputScreen.getText();
            int result = Integer.parseInt(i,16);
        ...
        btnLog.setVisible(false);
        btnfact = createButton("n!", columns[4], rows[4]);
        btnfact.addActionListener(event -> {
            ...
            inputScreen.setText(Integer.toString(fact));
        });
        btnfact.setVisible(false);
        
        btnexp = createButton("e", columns[4], rows[5]);
        btnexp.addActionListener(event -> {
            if (!Pattern.matches(DOUBLE_OR_NUMBER_REGEX, inputScreen.getText()))
                return;

            double i = Double.parseDouble(inputScreen.getText());
            double result = Math.exp(i);
            inputScreen.setText(Double.toString(result));
        });
        btnexp.setVisible(false);
        
        btnsin = createButton("sin", columns[2], rows[1]);
        ...
        btnsin.setFont(new Font("Comic Sans MS", Font.PLAIN, 24));
        btnsin.setVisible(false);
       
        
        btncos = createButton("cos", columns[3], rows[1]);
        ...
        btncos.setFont(new Font("Comic Sans MS", Font.PLAIN, 24));
        btncos.setVisible(false);
        
        btntan = createButton("tan", columns[3], rows[2]);
        ...
        btntan.setFont(new Font("Comic Sans MS", Font.PLAIN, 24));
        btntan.setVisible(false);

        btnupon = createButton("1/x", columns[3], rows[3]);
        btnupon.addActionListener(event -> {
            if (!Pattern.matches(DOUBLE_OR_NUMBER_REGEX, inputScreen.getText()))
                return;
        ...
        btnupon.setFont(new Font("Comic Sans MS", Font.PLAIN, 24));
        btnupon.setVisible(false);
